...
# CANALES CON REDUNDANCIA AUTOMÁTICA Y ALTA ESTABILIDAD
CHANNEL_FALLBACKS = {
    "espn2": [
    ...
    "universo": [
        f"{IPTV_SERVER_ALT}/live/{IPTV_USER}/{IPTV_PASS}/32038.ts",
    ],
    "disney1": [
        ...
        try:
            if "r=" in embed_iframe:
                ...
                if "stream=" in dec:
                    ...
                    if st_val in CHANNEL_FALLBACKS:
                        return st_val
        except Exception:
            pass
...
def resolve_live_stream_url(target):
    ...
    if target_clean in CHANNEL_FALLBACKS:
        sources = CHANNEL_FALLBACKS[target_clean]
        for src in sources:
...
def handle_message(msg):
    ...
    if text.startswith("/start") or text.startswith("/ayuda"):
        help_text = (
            ...
            "📺 <b>TRANSMITIR:</b>\n"
            "• <code>/stream espn2</code> $\\rightarrow$ Transmitir ESPN 2 Sur (Mariano Closs)\n"
            ...
            "• <code>/partidos</code> $\\rightarrow$ Ver <b>TODOS los partidos de rojadirectatv.ec</b> con todos sus canales\n"
...
def main():
    print("🤖 Bot Oficial y Exclusivo de rojadirectatv.ec listo...")
    offset = 0
    while True:
        try:
