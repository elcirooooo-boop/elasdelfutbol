...
# ==============================================================================
# 1. CONFIGURACIÓN DEL BOT Y CLAVE STREAM (FUENTE EXCLUSIVA: ROJADIRECTATV.WORLD)
# ==============================================================================
...
ROJADIRECTA_URL = "https://rojadirectatv.world/"
...
HEADERS = {
    ...
    "Referer": "https://rojadirectatv.world/"
}

# DECODIFICADOR GENÉRICO DE STREAMXHD
...
# RESOLVEDOR INTELIGENTE DE ENLACES DE ROJADIRECTATV.WORLD
def resolve_live_stream_url(target):
    target_clean = target.lower().strip()
    
    # 1. Si es ID tipo world_6 o canal-6 o número
    canal_id = None
    if target_clean.startswith("world_"):
        canal_id = target_clean.replace("world_", "")
    ...
    if canal_id:
        target_url = f"https://rojadirectatv.world/canal-{canal_id}.php"
    elif target.startswith("http"):
        target_url = target
    else:
        target_url = f"https://rojadirectatv.world/{target.lstrip('/')}"

    # 2. Extraer reproductor desde rojadirectatv.world
    try:
        r1 = requests.get(target_url, headers=HEADERS, timeout=6)
        if r1.status_code == 200:
            iframes_1 = re.findall(r'<iframe[^>]+src=["\']([^"\']+)["\']', r1.text)
            
            # Buscar iframe de rodrixtv o reproductor directo
            player_url = None
            ...
            if player_url:
                ...
                if r2.status_code == 200:
                    ...
                    # Si tiene un reproductor anidado como streamxhd, tvcstreams, etc.
    ...
    if target.endswith(".m3u8") or ".ts" in target:
        return target, f"Referer: {ROJADIRECTA_URL}\r\nOrigin: {ROJADIRECTA_URL.rstrip('/')}\r\n", True

    return None, f"No se pudo resolver el canal desde rojadirectatv.world: {target}", False

# EXTRACTOR DE LA AGENDA DIRECTA DE ROJADIRECTATV.WORLD
def get_live_agenda_messages(curr_key):
    try:
        r = requests.get(ROJADIRECTA_URL, headers=HEADERS, timeout=10)
        if r.status_code != 200:
            return ["🔴 No se pudo conectar a rojadirectatv.world en este momento."]
        ...
        messages = []
        header = f"📅 <b>AGENDA EXCLUSIVA ROJADIRECTA ({len(partidos_dict)} EVENTOS DE HOY)</b>\n\n"
        current_msg = header
        ...
        for (hour, title), links in partidos_dict.items():
            ...
            for idx, l in enumerate(links):
                ...
                cmd = f"/stream world_{cid} {curr_key}"
        ...
        return [f"⚠️ Error obteniendo la agenda de rojadirectatv.world: {e}"]
...
def handle_message(msg):
    ...
    if text.startswith("/start") or text.startswith("/ayuda"):
        help_text = (
            "⚽ <b>BOT DE TRANSMISIÓN EXCLUSIVO ROJADIRECTATV.WORLD</b>\n\n"
            "📺 <b>TRANSMITIR PARTIDO:</b>\n"
            "• <code>/partidos</code> $\\rightarrow$ Ver todos los partidos de hoy de <b>rojadirectatv.world</b> con opciones directas\n"
            "• <code>/stream world_6</code> $\\rightarrow$ Transmitir canal de la agenda\n"
        ...
        send_msg(chat_id, "⏳ <b>Cargando agenda exclusiva de rojadirectatv.world...</b>")
        ...
        if ok:
            send_msg(chat_id, (
                ...
                f"🌐 <b>Fuente:</b> rojadirectatv.world\n"
...
def main():
    print("🤖 Bot Conectado Exclusivamente a rojadirectatv.world listo...")
    offset = 0
    while True:
        try:
